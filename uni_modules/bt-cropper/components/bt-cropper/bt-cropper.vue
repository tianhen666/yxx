...
import { getTouchPoints, sleep, debounce, log } from './utils/tools.js'
...
export default {
	...
	computed: {
		...
		imageStyle() {
			let style = {
				...
				transform: `matrix(${this.imgScale}, 0, 0, ${this.imgScale}, ${this.imgTranslateX}, ${
					this.imgTranslateY
				}) translateZ(0px)`
			}
			return style
	...
	methods: {
		...
		imageZoom(
			...
			this.imgTranslateX = this.imgTranslateX + ((imagePosition.width * (scale - 1)) / 2) * (1 - offsetLeftPercent * 2)
			const offsetTopPercent = (center.top - imagePosition.top) / imagePosition.height
			this.imgTranslateY = this.imgTranslateY + ((imagePosition.height * (scale - 1)) / 2) * (1 - offsetTopPercent * 2)
		},
		onImageMove(ev) {
