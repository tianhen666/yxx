		...
		<view v-for="(item, index) in listData" :key="index" class="tab_content_item">
			...
			<view class="box">
				...
				<view class="btn">
					...
					<view class="btn_item style2" @tap="storeproductEnable(item, index)" v-if="item.status === 1">启用</view>
...
import { _storeproductGetlist, _storeproductEnable, _storeproductDisable, _storeproductDelete } from '@/api/yxxApi'
import { showModal, previewImage, navigateTo } from '@/utils/uniApp'
