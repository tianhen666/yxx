<template>
	<uni-popup ref="popupObj" type="bottom">
		<view class="login-container" v-if="useUserMain.headPortrait === 0">
			...
			<view class="login-top2">
				获取用户头像，昵称，手机号，主要用于向用户提供具有辨识度的用户中心页面，和订单生成
			</view>
			...
			>
				...
				<uni-forms-item
					:label="rules.avatar.label"
					name="avatar"
					v-else="compareVersion('2.9.7', SDKVersion) === 1"
				>
					<button
						class="avatar"
				...
				<!-- 	<uni-forms-item name="isAgree" label-width="0">
					<view class="box1">
						<radio
							...
							@tap.stop="formData.isAgree = !formData.isAgree"
						/>
						<view class="box1_item2">
							<text @tap="formData.isAgree = !formData.isAgree">我已阅读并同意</text>
							<text class="text1" @tap="navigateTo('/pages/main/agree/agree?text=1')">
								《软件使用协议》
							</text>
							<text class="text1" @tap="navigateTo('/pages/main/agree/agree?text=2')">
								《隐私协议》
							</text>
						</view>
					</view>
		...
		<view class="login-container" v-if="useUserMain.headPortrait === 1">
			...
			<view class="login-top2">
				获取用户手机号，主要用于向用户提供具有辨识度的用户中心页面，和订单生成
			</view>
			<view class="blank40"></view>
...
const rules = {
	...
	nickname: {
		rules: [
			{
				required: true,
				errorMessage: '请输入微信昵称'
			},
			{
				maxLength: 10,
				errorMessage: '请输入2-10字的中文'
			}
		],
		label: '微信昵称'
	},
