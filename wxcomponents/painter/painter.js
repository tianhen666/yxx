	...
	methods: {
		...
		getScaleIcon(rect, type) {
			...
			if (customActionStyle && customActionStyle.scale) {
				scaleArea = {
					type: 'image',
					url: type === 'text' ? customActionStyle.scale.textIcon : customActionStyle.scale
						.imageIcon,
					css: {
		...
		doAction(action, callback, isMoving, overwrite) {
			...
			if (newVal && newVal.url && doView.url && newVal.url !== doView.url) {
				...
				newVal && newVal.text && doView.text && newVal.text !== doView.text && (doView.text = newVal
					.text);
				newVal &&
					newVal.content &&
					doView.content &&
		...
		isInDelete(x, y) {
			for (const view of this.block.views) {
				if (view.id === 'delete') {
					return x > view.rect.left && y > view.rect.top && x < view.rect.right && y < view.rect
						.bottom;
				}
			}
			return false;
		...
		onClick() {
			...
			for (let i = totalLayerCount - 1; i >= 0; i--) {
				...
				if (this.touchedView && this.touchedView.id && this.touchedView.id === view.id && this
					.isInDelete(x, y, rect)) {
		...
		initDancePalette() {
			...
			this.downloadImages(this.properties.dancePalette).then(async palette => {
				...
				this.frontContext || (this.frontContext = await this.getCanvasContext(this
					.properties.use2D, 'front'));
				this.bottomContext || (this.bottomContext = await this.getCanvasContext(this
					.properties.use2D, 'bottom'));
				this.topContext || (this.topContext = await this.getCanvasContext(this.properties
					.use2D, 'top'));
				this.globalContext || (this.globalContext = await this.getCanvasContext(this
					.properties.use2D, 'k-canvas'));
				...
				function() {
					this.downloadImages(this.properties.palette).then(async palette => {
						...
						if (!this.photoContext) {
							this.photoContext = await this.getCanvasContext(this.properties
								.use2D, 'photo');
						}
						// 是否需要缩放
						if (needScale) {
								...
								path => {
									...
									wx.getImageInfo({
										...
										fail: error => {
											...
											console.warn(
												`getImageInfo ${view.originUrl} failed, ${JSON.stringify(error)}`
											);
											view.url = '';
										},
										complete: () => {
			...
			wx.getImageInfo({
				...
				success: infoRes => {
					if (that.paintCount > MAX_PAINT_COUNT) {
						const error =
							`The result is always fault, even we tried ${MAX_PAINT_COUNT} times`;
						console.error(error);
						that.triggerEvent('imgErr', {
							error: error,
						});
					...
					if (
						Math.abs(
							(infoRes.width * that.canvasHeightInPx - that.canvasWidthInPx * infoRes
								.height) /
...
function setStringPrototype(screenK, scale) {
	...
	String.prototype.toPx = function toPx(_, baseSize) {
		...
		if (formula && formula[1]) {
			// 进行 calc 计算
			const afterOne = formula[1].replace(/([^\s\(\+\-\*\/]+)\.(left|right|bottom|top|width|height)/g,
				word => {
