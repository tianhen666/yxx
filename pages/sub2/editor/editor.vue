...
import { navigateBackRefresh } from '@/aTemp/utils/uniAppTools.js'
...
function upload(src, type) {
	return new Promise((resolve, reject) => {
		...
		// console.log(globalConfig.BASE_URL + '/upload-flv/uploadimage')
		if (type === 'img') {
			// 上传图片
			uni.uploadFile({
				url: globalConfig.BASE_URL + '/upload-flv/uploadimage', // 接口地址
				filePath: src,
				formData: { baseDir: 'article' },
				name: 'file',
				success(res) {
					// console.log(res)
					const { code, data: imgUrl, msg } = JSON.parse(res.data)
					resolve(imgUrl) // 返回线上地址
				},
				fail: reject
			})
		} else if (type === 'video') {
			// 上传视频
			uni.uploadFile({
				url: globalConfig.BASE_URL + '/upload-flv/uploadVideo', // 接口地址
				filePath: src,
				formData: { baseDir: 'article' },
				name: 'file',
				success(res) {
...
export default {
	data() {
		return {
			content: uni.getStorageSync('artcleCentent'),
			modal: null,
			editable: true,
	...
	methods: {
		...
		edit(e) {
			...
			// console.log(e.currentTarget.dataset.method)
			...
			uni.showModal({
				...
				success: res => {
					if (res.confirm) {
						...
						// 移除缓存内容
						uni.setStorageSync('artcleCentent', '')
					}
				}
			})
		},
		...
		save() {
			setTimeout(() => {
				...
				// 设置设置缓存内容
				uni.setStorageSync('artcleCentent', content)
				navigateBackRefresh({ articleCentent: true })
				// uni.showModal({
				// 	title: '保存',
				// 	content,
				// 	confirmText: '完成',
