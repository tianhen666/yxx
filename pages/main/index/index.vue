...
<script setup>
	...
	import {
		_bannerList
	} from '@/aTemp/apis/banner'
	import {
		_storeGetinfo
	} from '@/aTemp/apis/store.js'
	import {
		_enrollformGetlist
	} from '@/aTemp/apis/activity.js'
	import {
		_serveGetlist
	} from '@/aTemp/apis/service.js'
	import {
		navigateTo
	...
	const getData = () => {
		...
		ListData.then(res => {
			...
			// 分享图片
			shareInfo.imageUrl =
				storeInfo.value.sharePic || storeInfo.value.pics ||
	...
	onLoad(async options => {
		...
		const parmsStr =
			`invitationCode=${invitationCode}&storeId=${storeId}&Mscene=${Mscene}&targetId=${targetId}`
		if (Mscene === 0) {
			// 跳转到目标--首页--页面
		} else if (Mscene === 1) {
			// 跳转到目标--活动--页面
