<template>
	<view class="container">
		<uni-forms :rules="rules" ref="formObj" v-model="formData" label-width="200rpx" err-show-type="toast">
			...
			<uni-forms-item :label="rules.avatar.label" name="avatar">
				<button open-type="chooseAvatar" class="avatar" @chooseavatar="onChooseAvatar"
					...
					<image :src="formData.avatar || '/static/images/default_avatar.png'" class="image"
			...
			<uni-forms-item :label="rules.nickname.label" name="nickname">
				<uni-easyinput type="nickname" :inputBorder="false" :clearable="false" trim v-model="formData.nickname">
				</uni-easyinput>
			</uni-forms-item>
		...
		<view class="box2">
			<button v-if="!formData.avatar || !formData.nickname || !formData.isAgree" class="btn btnAgree"
				:loading="loading" @tap="saveClick">
				微信授权登录
			</button>
