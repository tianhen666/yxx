	...
	<view class="fix-wrapper">
		<view class="fix">
			...
			<view class="button-container" v-if="false">
				...
				<view class="item" @tap.prevent.stop="recovery">
					<view class="icon">
						<uni-icons
							type="redo-filled"
							:color="
								posterDataListIndex < posterDataList.length - 1
									? icon.color
									: '#ffffff55'
							"
							:size="icon.size"
				...
				<!-- 文案 -->
				<view class="item" @tap.prevent.stop="copywriting">
					<view class="icon">
						<uni-icons type="pyq" :color="icon.color" :size="icon.size"></uni-icons>
					</view>
					<text>文案</text>
				</view>

				<!-- 暂存 -->
				<view class="item" @tap.prevent.stop="stagingPosterImg">
					<view class="icon">
						<uni-icons
				...
				<!-- 帮助 -->
				<view class="item" @tap.prevent.stop="mHelp">
					<view class="icon">
						<uni-icons
							type="help-filled"
							:color="icon.color"
			...
			<view class="button1-container">
				...
				<view class="item" v-if="!posterAdd">
					<button class="btn" @tap.prevent.stop="createPosterImg">
						生成海报并下载到手机
					</button>
				</view>
				...
				<!-- 暂处 -->
				<view class="item" @tap="stagingPosterImg" v-if="!posterAdd && !wenAn">
					<view class="icon">
						<uni-icons
							type="folder-add-filled"
							:color="icon.color"
				...
				<!-- 删除 -->
				<view class="item" @tap="delPosterImg" v-if="posterAdd && posterOtherData.value.id">
					<view class="icon">
						<uni-icons
							type="folder-add-filled"
							:color="icon.color"
...
import {
	_posterRenewalPosterImg,
	_posterSavePostLog,
	_posterDeletePosterImg
...
// 是否从文案进入
const wenAn = inject('wenAn', false);
...
const posterRenewalPosterImg = () => {
	...
	return _posterRenewalPosterImg({
		...
		posterName: !drafts.value
			? posterOtherData.value.postercampaign
			: posterOtherData.value.posterName,
		posterUrl: !drafts.value
			? posterOtherData.value.posterurl
			: posterOtherData.value.posterUrl,
...
const handleAdd = () => {
	// 重新开始记录
	posterDataList.value = JSON.parse(
		JSON.stringify(posterDataList.value.slice(0, posterDataListIndex.value + 1))
	);
