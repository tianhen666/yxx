<template>
  ...
  <view class="container" v-else>
    ...
    <view
      class="related_goods"
      v-if="dataObj.productList && dataObj.productList?.length > 0"
    >
  ...
  if (dataId.value > 0) {
    ...
    _serveGetinfo({ id: dataId.value }).then((res) => {
      ...
      shareInfo.path = computed(
        () =>
          `/pages/main/index/index?invitationCode=${useUserMain.userid}&storeId=${useUserMain.storeId}&Mscene=3&targetId=${dataObj.value.id}`
      );
      shareInfo.imageUrl =
      ...
      shareInfo.query = computed(
        () =>
          `invitationCode=${useUserMain.userid}&storeId=${useUserMain.storeId}&Mscene=3&targetId=${dataObj.value.id}`
      );
    });
  }
